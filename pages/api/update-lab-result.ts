...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (
    (typeof validBody.positive === 'boolean' || typeof validBody.needsRetest === 'boolean') &&
    validBody.sampleCode !== undefined
  ) {
    return res.status(400).end()
    // this is also responsible for updating needs_retest prop, TODO: refactor this endpoint
  } else if (
    typeof validBody.positive === 'boolean' ||
    typeof validBody.needsRetest === 'boolean'
  ) {
    await client.UpdateLabResultPositiveMutation(validBody)
